...
test("reveals every section immediately for tab mode or missing observer", () => {
  const state = { leaderboard: false, bestlaps: false, safety: false, clubsTeams: false };
  createHomeDeferredSectionsController({ initialState: state }).setup({ isHome: true, tabsEnabled: true, windowRef: {}, documentRef: {} });
  assert.deepEqual(state, { leaderboard: true, bestlaps: true, safety: true, clubsTeams: true });
});
...
test("does nothing outside home", () => {
  const state = { leaderboard: true, bestlaps: true, safety: true, clubsTeams: true };
