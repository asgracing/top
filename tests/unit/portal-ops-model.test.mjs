...
import { hourlyDraftEquals, isSafeLogoPreview, normalizeHourlyState, snapshotHourlyDraft, validateClubsCommand, validateHourlyDraft } from "../../src/pages/portal-ops/portal-ops-model.js";
...
const approvedDraft=snapshotHourlyDraft(validateHourlyDraft(valid,["spa"]).value);
const changedDraft={...approvedDraft,track_code:"monza"};
assert.equal(approvedDraft.track_code,"spa");
assert.equal(hourlyDraftEquals(approvedDraft,{...approvedDraft}),true);
assert.equal(hourlyDraftEquals(approvedDraft,changedDraft),false);
