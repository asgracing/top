...
test("declares every home statistics table view state", () => {
  assert.deepEqual(HOME_TABLE_VIEW_STATES.map(item => item.tableId), ["leaderboard-table", "bestlaps-table", "safety-table", "clubs-teams-home-table"]);
...
test("applies localized error state to every table", () => {
  ...
  assert.deepEqual(calls.map(call => call[2]), ["t:errorLeaderboard", "t:errorBestlaps", "t:errorLoading", "t:clubsTeamsUnavailable"]);
});
