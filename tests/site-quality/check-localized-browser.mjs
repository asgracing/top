...
try {
  for (const path of ["/", "/hourly/", "/hourly/championship/", "/teams/", "/community/"]) {
    ...
    assert.deepEqual(current.errors, [], `runtime errors: ${path}`);
    ...
    assert.deepEqual(ru.errors, [], `RU runtime errors: ${path}`);
