...
for (const [page, [htmlPath, entrySrc]] of Object.entries(pageEntrypoints)) {
  ...
  const baseHref = page === "home" ? "./styles/base.css?v=" : "../styles/base.css?v=";
  ...
  for (const href of [
    page === "home"
      ? "./styles/components/driver-day-modal.css?v=20260826previewtitle1"
...
if (!html.includes('./styles/components/floating-widgets.css?v=20260820widgetcollapse8')) failures.push("Home must load the floating widget coordination component");
...
if (!floatingWidgetsCss.includes("top: calc(var(--top-nav-height, 64px) + 8px);") || !floatingWidgetsCss.includes("max-height: calc(100dvh - var(--top-nav-height, 64px) - 16px);") || !floatingWidgetsCss.includes(".donation-alerts-widget:not(.is-collapsed)")) failures.push("Expanded mobile widgets must remain fully below the measured header");
...
if (!html.includes('./styles/components/hero-layout.css?v=20260909eventwrap1') || !heroLayoutCss.includes("@media (min-width: 1181px)")) failures.push("Home must load the extracted responsive hero layout component");
if (!authHeaderCss.includes("@media (min-width: 721px) and (max-width: 1180px)") || !authHeaderCss.includes("grid-template-columns:repeat(2,minmax(0,1fr))")) failures.push("Tablet profile metrics must use a two-by-two grid");
if (!heroLayoutCss.includes("@media (min-width: 641px) and (max-width: 1180px)") || !heroLayoutCss.includes("grid-template-columns: repeat(2, minmax(0, 1fr));") || !heroLayoutCss.includes("grid-template-columns: minmax(0, 1fr) minmax(160px, auto) 96px;")) failures.push("Tablet hero must keep sequential sections, paired hourly cards, and a bounded support row");
...
if (!js.includes('serverGroupHourly:') || !js.includes('Special event server') || !js.includes('Сервер спец. ивента') || !js.includes('getServerStatusGroups(items)')) failures.push("Server widget must localize and group the special event, primary and secondary servers explicitly");
if (js.includes('style="--server-card-bg:') || !serverStickyLayoutCss.includes(".server-sticky-card-metrics")) failures.push("Server widget must use readable structured metrics without track background images");
if (!js.includes('getServerSrRequirement(key, server)') || !js.includes('srRequirement.toFixed(1)') || !serverStickyLayoutCss.includes(".server-sticky-metric-access")) failures.push("Server widget must show compact SA and internal SR access requirements");
...
const budgets = {
  ...
  mediaQuery: [(css.match(/@media\b/g) || []).length, 69],
