...
Статус: `Backlog`
Область: главная страница `/top`, блок рейтингов

Добавить рядом с существующими таблицами рейтинга, быстрых кругов и безопасности отдельную вкладку `ELO` с теми же механикой, визуальным оформлением, поиском, сортировкой, пагинацией и карточкой пилота.

Рекомендуемые колонки:

1. Позиция в рейтинге ELO.
2. Имя, фамилия и тег пилота.
3. ELO по убыванию с бейджем категории.
4. SR.
5. Форма ELO за последние 10 гонок — среднее `rating_delta`, рассчитанное фронтендом.
6. Количество гонок, учтённых в ELO.
7. Лучший круг.
8. Машина лучшего круга.

Дополнительно фронтенд может рассчитывать средний ELO за карьеру как среднее `new_rating` из истории, но основной рекомендуемый показатель — форма ELO за 10 гонок, поскольку она лучше показывает текущую динамику пилота.

Данные уже присутствуют в экспорте: ELO, категория и история ELO, SR, лучший круг и машина. Для корректной глобальной позиции и производительности сформировать из этих же данных отдельный компактный экспорт:

- `top-data/v2/tables/elo.json`;
- `top-data/v2/tables/elo/page-{page}.json`.

Экспорт должен:

- включать только пилотов с опубликованным ELO после калибровки;
- сортировать строки по ELO от большего к меньшему;
- присваивать глобальный `elo_rank`;
- использовать только безопасный `public_id`, без Steam ID;
- содержать компактную историю `new_rating` и `rating_delta`, необходимую для фронтенд-расчётов;
- поддерживать загрузку страниц без скачивания общего `leaderboard.json`.
...
| R15 | Готово | Allowlist `dist`, asset manifest/SHA-256/build metadata, rollback artifact, CI quality gate и HTTP deployment smoke | R01–R14 |
| R16 | Готово, ветка опубликована | Полный regression, документация, PR template и rollout/rollback готовы; feature-ветка опубликована по SSH, draft PR ожидает открытия | R15 |
...
- 12.07.2026 — R10 начат: зафиксирован исходный монолит `app.js` размером 512966 bytes и восемь обслуживаемых им маршрутов. Home, races, driver, cars, fun-stats, community, news и bans получили отдельные ES-module entrypoints; общий `legacy-bootstrap` временно загружает неизменённую бизнес-логику и помечает активную страницу через `data-page`. Quality gate запрещает прямое подключение `app.js` на этих страницах и проверяет соответствие route → entrypoint.
...
- 13.07.2026 — R10: Cars columns и обработка строк вынесены в `src/pages/cars/model.js`; фиктивный `filterCars` удалён, а page model использует общий sorter через явную зависимость. Добавлены три unit-теста, baseline расширен до 109 тестов.
- 13.07.2026 — R10: Cars table renderer вынесен в lazy `src/pages/cars/table-view.js`; loading/empty/table states, row markup, fastest-lap highlight и sort binding отделены от `app.js`. Добавлены три unit-теста, baseline расширен до 112 тестов.
...
- 13.07.2026 — R11 hotfix: runtime заменяет стартовый класс `main` на track key (`monza`, `spa`, `nurburgring`, `nurburgring24h`, `silverstone`). Для каждого поддерживаемого класса теперь задан собственный CSS background fallback; отображение изображения больше не зависит от наличия inline style после динамического render.
- 13.07.2026 — R11 hotfix: track classes переведены с одного custom-property fallback на явный двухслойный `background-image` (контрастный gradient + JPG). Двойное затемнение overlay ослаблено; миниатюры трасс остаются читаемыми под белым текстом и не зависят от вычисления inline `url()`.
...
## Журнал выполнения — 15.07.2026

- R11: компактная сетка правого серверного виджета вынесена из `styles.css` в `styles/components/server-sticky-layout.css`.
- Новый компонент подключён на всех восьми основных маршрутах.
...
- 15.07.2026 — повторный R12 hotfix mini stats: вместо `display: contents` закреплена явная двухуровневая сетка — верхний ряд «Серверы онлайн / Лучшие круги», нижний ряд «Всего игроков / Пилотов в рейтинге». На desktop используются четыре равные карточки 2×2 высотой 58 px; мобильный переход остаётся одноколоночным.
...
- 15.07.2026 — R15 завершён: `npm run build` собирает `dist/` только из allowlist runtime-файлов, исключая tests/scripts/workers/stats tool, source maps, логи, Markdown и backup-медиа. Артефакт содержит `asset-manifest.json`, SHA-256 `checksums.sha256` и `build-meta.json` с revision/time/rollback location; повторная сборка сохраняет рабочую предыдущую версию в `dist.previous/`. `verify:dist` проверяет состав, manifest и checksums, `smoke:dist` поднимает HTTP-сервер из `dist` и проверяет 12 маршрутов/ресурсов плюс byte-range видео. GitHub Actions выполняет единый `npm run ci` и хранит versioned artifact 14 дней. Локальный итог: 217 файлов, 195/195 unit-тестов, все quality/performance/dist/smoke gates зелёные.
- 15.07.2026 — R16 локально завершён: добавлен dist regression для локальных HTML/CSS/JS references (545 ссылок), который обнаружил и позволил исправить неверный путь hero crown asset. Подготовлены итоговый отчёт, обновлённый README, PR template и пошаговый rollout/rollback с ручной RU/EN desktop/mobile матрицей. Полный `npm run ci` зелёный: 195/195 тестов, quality/performance budgets, allowlist artifact/checksums, reference regression и HTTP smoke. Свежие автоматические screenshots не получены: встроенный Browser не предоставляет доступных browser instances. Публикация draft PR заблокирована отсутствием `gh` и GitHub HTTPS credentials; ветка подготовлена локально без изменений `main`.
- 15.07.2026 — R16 publication update: SSH-доступ GitHub подтверждён для аккаунта `asgracing`, `origin` переключён на `git@github.com:asgracing/top.git`, ветка `refactor/top-home-backlog-2026-07-12` опубликована и получила upstream. `main` не изменялся; draft PR можно открыть из опубликованной ветки, CLI-создание PR остаётся недоступным без `gh`.
