...
## P0 / самый высокий приоритет с 10.08.2026
...
## После P0 — пакетные приглашения в состав

- [ ] множественный выбор пилотов в управлении составом клуба или команды;
- [ ] одна кнопка и одно подтверждение для отправки до 20 приглашений;
- [ ] отдельная backend-команда на каждого получателя, чтобы частичный отказ не
  отменял успешно поставленные приглашения;
- [ ] исключение из поиска действующих участников и пилотов с уже отправленным
  приглашением;
- [ ] итог пакетной отправки: сколько приглашений поставлено в очередь и какие
  получатели были отклонены;
- [ ] отображение имени получателя в исходящих заявках и приглашениях;
- [ ] сохранение `subject_display_name` в allowlisted actor-state с обратной
  совместимостью старого snapshot-контракта;
- [ ] отдельные статусы для каждого получателя: queued, pending, accepted,
  rejected, expired;
- [ ] одно обновление кабинета после постановки всего пакета, без ожидания и
  подтверждения каждой команды по отдельности;
- [ ] RU/EN, keyboard navigation, mobile layout и тест частично успешной пачки.
